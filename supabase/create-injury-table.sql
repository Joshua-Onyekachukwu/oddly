...
-- Add injury columns to team_strengths if missing
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='team_strengths' AND column_name='xg_features') THEN
    ALTER TABLE team_strengths ADD COLUMN xg_features JSONB;
  END IF;
END $$;
